...
//    	
//    	while (!pq.isEmpty()) {
//    		// 우선순위 큐에서 현재 가장 작은 시간을 가진 컴퓨터를 꺼냅니다.
//    		Point cur = pq.poll();
//    		int curTime = cur.time; // 현재 컴퓨터의 시간
//    		int curComputer = cur.y; // 현재 컴퓨터의 번호
//    		
//    		// 이미 최소 시간으로 갱신된 경우, 더 이상 처리하지 않고 넘어갑니다.
//    		if (curTime > timeToInfest[curComputer]) {
//    			continue;
//    		}
//    		
//    		// 현재 컴퓨터와 연결된 모든 컴퓨터에 대해 처리합니다.
//    		for (Point next : adjList[curComputer]) {
//    			int nextComputer = next.y; // 다음 컴퓨터 번호
//    			int nextTime = curTime + next.time; // 다음 컴퓨터로 가는데 걸리는 시간
//    			
//    			// 다음 컴퓨터로 가는 경로가 현재까지의 최소 시간보다 짧은 경우
//    			if (nextTime < timeToInfest[nextComputer]) {
//    				// 최소 시간을 업데이트하고 다음 컴퓨터를 우선순위 큐에 추가합니다.
//    				timeToInfest[nextComputer] = nextTime;
//    				pq.add(new Point(nextComputer, nextTime));
//    			}
//    		}
//    	}
